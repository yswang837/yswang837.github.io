...
&emsp;&emsp;在项目中选择调用/返回风格的原因是为了良好的可修改性、可重用性和可扩展性。我们将该系统整体分成了4层的B/S和C/S架构，分别是表示层、控制层、业务逻辑处理层、数据持久层；表示层通常负责接收用户的请求，对用户的输入、系统的输出进行检查与控制，包括页面跳转、向用户呈现最终的结果信息等；控制层主要负责跨域、接收请求、验签、鉴权、日志追踪以及返回数据等；业务逻辑处理层是对业务功能的抽象，它把具体的业务功能抽象成控制层可调用、可加工的资源，这类资源通常来自于数据库的多个相关的表，为了更快速地查询数据，一些高频数据通过业务逻辑处理层保存到缓存数据库中；数据层主要采用Gorm和Redisgo来完成对数据的、单一职责的增、删、改和查操作，其主要功能是为业务逻辑层提供透明的数据访问、持久化等能力。底层为上层提供服务，上层为底层提供驱动，修改当前层，最多只会影响逻辑上紧挨的上下两层。

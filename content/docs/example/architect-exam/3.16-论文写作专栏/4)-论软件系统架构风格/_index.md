...
&emsp;&emsp;在项目中选择调用/返回风格的原因是为了良好的可修改性、可重用性和可扩展性。我们将该系统整体分成了4层分层架构，分别是表示层、控制层、业务逻辑层、数据持久层；数据层主要采用Gorm和Redisgo来完成对数据的、单一职责的增、删、改和查操作；底层为上层提供服务，上层为底层提供驱动，修改当前层，最多只会影响紧挨的上下两层

...
&emsp;&emsp;调用/返回风格包含主程序/子程序、面向对象和层次架构子风格。主程序/子程序风格是结构化程序设计的一种典型风格，通过逐步分解和细化形成整个系统的架构。面向对象风格将系统看做是对象的集合，通过函数调用来交互，包含封装、继承和多态等特性。层次架构风格将信息系统进行分层，保证了系统的可维护性和可扩展性。
...
&emsp;&emsp;仓库风格有两种子风格：数据库系统和黑板系统。仓库风格的软件元素分为两类：资源库和相互依赖的构件组。资源库用于表示系统的当前状态，构件组可以对资源库进行操作。资源库和构件之间可以进行信息交换，这是实现数据共享体系结构的技术基础。根据控制策略的不同，数据共享体系结构可以分为传统的数据库风格和黑板风格。如果系统的信息处理是由事件驱动并将结果存储到资源库中，则属于数据库风格。如果系统的运行是由资源库的当前状态驱动，则属于黑板风格。
...
&emsp;&emsp;仓库风格在项目中主要体现在数据库系统方面，

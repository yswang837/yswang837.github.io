...
冷热redis、过期时间、长链接的连接池、分布式集群、读写分离、主从同步、接口限制、计数器

&emsp;&emsp;Redis主要用于商业化广告子系统中。该系统的痛点主要是峰值QPS高达7万以及每日约2.6亿的海量数据；对此，我们采用了读写分离、主从同步的分布式集群来应对，由于业务的复杂性，我们为该系统设计了19个Redis实例，其中“用户与设备关系”是读写量最大的实例，包含了64个端口，每个端口都使用了长链接的连接池保持服务的性能和稳定性；对每个实例中的数据都设置了合理的过期时间，保证了热数据均在缓存中，冷数据被自动删除。我们利用Redis自增key的特性，对系统中各个重要功能做了接口调用的频次限制，以保证接口调用的合理性；比如说：在1小时内，同一个IP调用该接口不能超过3600次；在1天内不能超过5万次。此外

&emsp;&emsp;HBase主要用于数据仓库子系统中。

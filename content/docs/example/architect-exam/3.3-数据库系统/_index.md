...
&emsp;&emsp;分库分表通常是针对mysql；当数据库的QPS过高、数据库的连接数不足的时候就需要分库；当单表数据量过大，严重影响读写性能时就需要分表。分表有水平分表和垂直分表；水平分表：将数据记录存放在不同的表中；垂直分表：将不同的字段放在不同的表中。

&emsp;&emsp;分库分表同样可采用、范围、列表和哈希，通常采用哈希算法。
...
&emsp;&emsp;分布式数据库是对应于集中式数据库而言的，平时所用到的简单的mysql等都是集中式数据库(在企业中基本上也是用mysql来做分布式数据库的)，分布式会考虑物理上把数据放在不同的物理节点上，其逻辑上依然是一个整体。
...
&emsp;&emsp;分布式数据库的层次和集中式数据库的三级模式两级映像大致相似，只不过在原有的外模式、概念模式、内模式上增加了一些具体的特性，如分片模式有(垂直分片、水平分片、混合分片)，分布模式(表示具体放在哪个物理节点)；全局外模式和外模式一样，都直接用于用户程序，局部概念模式和概念模式一样，都是库表，局部内模式同内模式一样，其具体情况如下图：
...
- 水平分片：是将表中不同行的数据存储到不同的服务器上。
- 垂直分片：是将表中不同字段的数据存储到不同的服务器上。

### 2.4 分布模式 ✅✅✅

&emsp;&emsp;分布模式使得数据具有分布透明性；具体分为：复制透明性、位置透明性。

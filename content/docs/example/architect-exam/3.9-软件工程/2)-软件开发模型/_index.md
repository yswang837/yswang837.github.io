...
&emsp;&emsp;CORBA标准中，OMG接口定义语言IDL文件包含了六种不同的元素，包括模块定义、类型定义、常量定义、异常、接口描述和值类型，其中，**接口描述是一个IDL文件最核心的内容**，**模块定义**将被映射为Java语言中的**包**和C++语言中的**命名空间**。CORBA服务端构建模型中，**伺服对象(Servant)**是CORBA对象的**真正实现**，负责完成客户端请求，**可移植对象适配器POA**的作用是**在底层传输平台与接收调用并返回结果的对象实现之间进行协调**。COM支持两种形式的对象组装：**包含**是一个对象拥有指向另一个对象的唯一引用、**聚集**是直接把内部对象接口用传给外部对象的客户，而不是再转发请求。

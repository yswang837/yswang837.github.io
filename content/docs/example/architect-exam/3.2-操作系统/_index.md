...
&emsp;&emsp;断处理相关的时间指标：中断延迟时间、中断响应时间
...
&emsp;&emsp;轮转调度)最晚截止期调度算法:指调度程序按每个任务的最接近其截止期末端的时间进行调度。最早截止期调度算法，指调度程序按每个任务的截止期时间，选择最早到截止期头端时间的任务进行调度。
...
&emsp;&emsp;将传统的操作系统代码放置到更高层，从操作系统中去掉尽可能多的东西，而只留下最小的核心，称之为微内核。(C/S结构)
